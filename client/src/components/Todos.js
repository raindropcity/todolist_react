...
export default function Todos(props) {
  ...
  const [informationOfSeletedTodo, setInformationOfSeletedTodo] = React.useState({ id: props.todo._id, urgent: props.todo.urgent })
  ...
  React.useEffect(() => {
    ...
    async function sendReq() {
      try {
        setInformationOfSeletedTodo((prev) => {
          return {
            ...prev,
            urgent: !prev.urgent
          }
        ...
        await axios.put('/api/changeUrgentState', { id: informationOfSeletedTodo.id, urgent: informationOfSeletedTodo.urgent })
      } catch (err) {
        console.log(err)
      }
    }
  ...
  }, [informationOfSeletedTodo, starIsClicked, props])
  ...
  React.useEffect(() => {
    ...
    async function sendReq() {
      try {
        ...
        const dataFromBackend = await axios.put('/api/deleteTodo', { id: informationOfSeletedTodo.id })
  ...
  }, [informationOfSeletedTodo.id, checkIsClicked])
  ...
  return (
    <div className="each-todo" >
      <Link className="each-todo-leftpart" to={`/todo/edit/${props.todo._id}`} style={{ textDecoration: "none", color: "#cfd3d3" }}>
        {informationOfSeletedTodo.urgent && <i className="fas fa-circle"></i>}
      ...
      {!deleteIsClicked && (
        <div className="each-todo-rightpart">
          <i className="far fa-star" style={{ color: informationOfSeletedTodo.urgent ? "#ecac0c" : "#cfd3d3" }} onClick={() => setStarIsClicked(true)}></i>
